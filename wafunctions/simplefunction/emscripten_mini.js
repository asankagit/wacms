...
var wasmBinaryFile;
  wasmBinaryFile = 'emscripten_mini.wasm';
...
function __asyncjs__do_fetch3(url,lenurl){ return Asyncify.handleAsync(async () => { out("waiting for a fetch"); call_mine(UTF8ToString(url, lenurl)); const response = await fetch(UTF8ToString(url, lenurl)).then(res => res.buffer()) .then(json => json.toString());; out("got the fetch response"); const lengthBytes = lengthBytesUTF8(response)+1; const stringOnWasmHeap = _malloc(lengthBytes); stringToUTF8(response, stringOnWasmHeap, lengthBytes); return stringOnWasmHeap; }); }
function __asyncjs__sleeps(url,lenurl){ return Asyncify.handleAsync(async () => { const promise = new Promise((reslv, rej) => { setTimeout(() => reslv('ok'), 25000); }); const wakedupMsg = await promise; return true; }); }
...
var asmLibraryArg = {
  ...
  "__asyncjs__do_fetch3": __asyncjs__do_fetch3,
  "__asyncjs__sleeps": __asyncjs__sleeps,
