...
function __asyncjs__wafuz_fetch(url,lenurl){ return Asyncify.handleAsync(async () => { out("waiting for a fetch"); call_mine(UTF8ToString(url, lenurl)); const response = await fetch(UTF8ToString(url, lenurl)).then(res => res.buffer()) .then(json => json.toString());; out("got the fetch response"); const lengthBytes = lengthBytesUTF8(response)+1; const stringOnWasmHeap = _malloc(lengthBytes); stringToUTF8(response, stringOnWasmHeap, lengthBytes); return stringOnWasmHeap; }); }
...
var asmLibraryArg = {
  ...
  "__asyncjs__wafuz_fetch": __asyncjs__wafuz_fetch,
