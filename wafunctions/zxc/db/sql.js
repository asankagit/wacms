...
class SQLHelper {
    ...
    init  = async (wafuz_name) => {
        const dbDumpBuffer = fs.readFileSync(path.join(__dirname, "./db.sqlite"));
        const sql = await createSqlWasm({ wasmUrl: path.join(__dirname,"./db/sqlite3.wasm") });
        // From here on, the SQL.js API can be used...
    ...
    db_exec = async (db, queryString = "SELECT * FROM hello") => {
        console.log("dab", db)
        var res = await this.db.exec(queryString);
    ...
    persist = () => {
        ...
        const buffer = new Buffer.from(data);
        fs.writeFileSync(path.join(__dirname,"./db.sqlite"), buffer);
    }
}
// persist()
