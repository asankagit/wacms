...
function __asyncjs__do_fetch(num){ return Asyncify.handleAsync(async () => { out("waiting for a fetch"); Module.timer = false; call_mine(num); const response = await fetch("http://api.plos.org/search?q=title:DNA").then(res => res.buffer()) .then(json => json.toString());; out(response); out("got the fetch response"); Module.timer = true; return response; }); }
